...
def getOutput(status):
    ...
    con = pymysql.connect(host='localhost',port = 3306,user = 'root', password = 'root', database = 'evoting',charset='utf8')
    with con:
        cur = con.cursor()
...
def getCount(name):
    ...
    for i in range(len(blockchain.chain)):
        if i > 0:
            ...
            arr = data.split("#")
            if arr[1] == name:
                count += 1  # Increment count for each transaction found
    return count
    ...
    if request.method == 'GET':
        ...
        with con:
            ...
            for row in rows:
                ...
                # Append party names and their vote counts for graph
        ...
        # Generate bar graph
        plt.bar(parties, vote_counts, color='blue')
        plt.xlabel('Party Name')
        plt.ylabel('Vote Count')
        plt.title('Vote Count by Party')
        plt.xticks(rotation=45)  # Rotate x-axis labels for better visibility
        plt.tight_layout()  # Save the graph as an image# Specify the directory path
        directory = 'EVotingApp/static/images'
        # Ensure that the directory exists, create it if it doesn't
        os.makedirs(directory, exist_ok=True)
        # Construct the file path using os.path.join
        file_path = os.path.join(directory, 'vote.png')
...
def AddPartyAction(request):
    if request.method == 'POST':
      ...
      db_connection = pymysql.connect(host='localhost',port = 3306,user = 'root', password = 'root', database = 'evoting',charset='utf8')
